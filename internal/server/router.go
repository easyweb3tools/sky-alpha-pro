...
func NewRouterWithServices(cfg *config.Config, log *zap.Logger, db *gorm.DB, metricReg *metrics.Registry, services *Services, schedulerMgr *scheduler.Manager) http.Handler {
	...
	router.GET("/ops/agent/prompt-performance", OpsAgentPromptPerformanceHandler(db))
	router.GET("/ops/agent/strategy-changes", OpsAgentStrategyChangesHandler(db))
	router.GET("/ops/agent/strategy-params", OpsAgentStrategyParamsHandler(db))
	router.GET("/ops/agent/sessions/:id/replay", OpsAgentSessionReplayHandler(db))
	router.GET("/ops/inspection", OpsInspectionHandler(db, metricReg, schedulerMgr))
	router.GET("/ops/events/summary", OpsEventsSummaryHandler(services.Opportunity))
	router.GET("/ops/candidates/summary", OpsCandidatesSummaryHandler(services.Opportunity))
	router.GET("/ops/candidates/transitions", OpsCandidateTransitionsHandler(services.Opportunity))
	router.POST("/ops/events/emit", OpsEmitEventHandler(services.Opportunity))
	...
	api := router.Group("/api/v1")
	{
		...
		api.GET("/ops/agent/prompt-performance", OpsAgentPromptPerformanceHandler(db))
		api.GET("/ops/agent/strategy-changes", OpsAgentStrategyChangesHandler(db))
		api.GET("/ops/agent/strategy-params", OpsAgentStrategyParamsHandler(db))
		api.GET("/ops/agent/sessions/:id/replay", OpsAgentSessionReplayHandler(db))
		api.GET("/ops/inspection", OpsInspectionHandler(db, metricReg, schedulerMgr))
		api.GET("/ops/events/summary", OpsEventsSummaryHandler(services.Opportunity))
		api.GET("/ops/candidates/summary", OpsCandidatesSummaryHandler(services.Opportunity))
		api.GET("/ops/candidates/transitions", OpsCandidateTransitionsHandler(services.Opportunity))
		api.POST("/ops/events/emit", OpsEmitEventHandler(services.Opportunity))
